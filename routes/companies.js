...
router.get('/', async function (req, res, next) {
	...
	// converts query string values to integers
	...
	try {
		// validate the query string input against the schema
		const validator = jsonschema.validate(filters, companySearchSchema);
		if (!validator.valid) {
			const err = validator.errors.map((e) => e.stack);
			throw new BadRequestError(err);
		}
		// once validation is complete, can make the query with the filters passed in
